...
export default function ManageCarTypes() {
  ...
  const handleDeleteClick = async (id, name) => {
    ...
    try {
      ...
      const inUse = allCars.some((c) => String(c.carTypeId) === String(id))
      if (inUse) {
        setDeleteError(`Cannot delete "${name}" because it is currently used by some cars.`)
      } else {
        setDeleteError(null)
      }
      setShowModal(true)
    } catch {
      alert('Error verifying car type usage.')
    }
  }
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      ...
      setCarTypes(carTypes.filter((rt) => rt.id !== targetId))
      setShowModal(false)
    } catch {
