    ...
    <Card className="mx-auto" style={{ maxWidth: 480 }}>
      ...
      <Card.Body>
        ...
        <ButtonGroup className="w-100 mb-3">
          ...
          <Button variant="danger" data-testid="btn-decrement" onClick={() => dispatch({ type: 'DECREMENT' })}>
            −
          </Button>
          {/* TODO 7: dispatch RESET */}
          <Button variant="secondary" data-testid="btn-reset" onClick={() => dispatch({ type: 'RESET' })}>
            Reset
          </Button>
          {/* TODO 8: dispatch INCREMENT */}
          <Button variant="success" data-testid="btn-increment" onClick={() => dispatch({ type: 'INCREMENT' })}>
            +
          </Button>
        </ButtonGroup>
        ...
        <div>
          ...
          <div data-testid="history-list" className="mt-1 d-flex flex-wrap gap-1">
            {state.history.map((item, index) => (
              <Badge key={index} bg="secondary" data-testid="history-item">
                {item}
              </Badge>
            ))}
          </div>
